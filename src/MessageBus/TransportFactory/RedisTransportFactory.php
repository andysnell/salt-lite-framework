...
class RedisTransportFactory
{
    ...
    public function make(string $connection, array $options): RedisTransport
    {
        ...
        return Reflect::ghost(RedisTransport::class, function (RedisTransport $ghost) use ($connection, $options): void {
            $ghost->__construct(new RedisTransportConnection(
                $options,
                $this->redis_manager->connect($connection),
            ));
        });
    }
}
