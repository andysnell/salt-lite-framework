...
final readonly class Paseto implements Stringable
{
    ...
    public static function local(PasetoKey $key, PasetoMessage $message): self
    {
        ...
        $ciphertext = \sodium_crypto_aead_xchacha20poly1305_ietf_encrypt(
            $message->data,
            self::pae($header, $nonce, $message->footer),
            $nonce,
            $key->shared()->bytes(),
        );
    ...
    public static function public(PasetoKey $key, PasetoMessage $message): self
    {
        ...
        $encoded = self::pae($header, $message->data, $message->footer, $message->implicit_claims);
        $signature = \sodium_crypto_sign_detached($encoded, $key->secret()->bytes());
    ...
    private function verify(PasetoKey $key, string $implicit = ''): PasetoMessage
    {
        ...
        $encoded = self::pae($header, $data, $this->footer, $implicit);
        if (\sodium_crypto_sign_verify_detached(NonEmpty::string($signature), $encoded, $key->public()->bytes())) {
    ...
    private function decrypt(PasetoKey $key): PasetoMessage
    {
        ...
        $data = \sodium_crypto_aead_xchacha20poly1305_ietf_decrypt(
            ...
            self::pae($header, $nonce, $this->footer),
            $nonce,
            $key->shared()->bytes(),
        );
